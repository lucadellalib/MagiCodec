...
def _flash_attn_forward_fake(
    ...
    if return_softmax:
        p = torch.empty((batch_size, num_heads, round_multiple(seqlen_q, 128), round_multiple(seqlen_k, 128)),
...
def _flash_attn_backward_fake(
    ...
    softmax_d = torch.empty((batch_size, num_heads, round_multiple(seqlen_q, 128)), device=q.device,
                            dtype=torch.float32)
...
@_torch_custom_op_wrapper("flash_attn::_flash_attn_varlen_backward", mutates_args=("dq", "dk", "dv"),
                          device_types="cuda")
